...
def _panel_series_map(df: pd.DataFrame, shift: int) -> pd.Series:
    """For each row, look up log_volume at continuous_week - shift within same panel unit.
    ...
    right = sorted_df[PANEL_KEYS + [TIME_COL, "log_volume"]].rename(
        columns={TIME_COL: "_target_cw", "log_volume": "_lag_val"}
    )
...
def naive_predict(train_df: pd.DataFrame, val_df: pd.DataFrame) -> np.ndarray:
    ...
    # fallbacks
    panel_mean = train_df.groupby(PANEL_KEYS)["log_volume"].mean()
    global_mean = float(train_df["log_volume"].mean())
