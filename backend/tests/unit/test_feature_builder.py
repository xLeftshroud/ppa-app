...
def test_batch_prices_produces_n_rows() -> None:
    ...
    df = build_feature_df(prices, "L2_TESCO", 0, 13, {}, continuous_week=100)
    assert len(df) == 4
...
def test_week_encoding_matches_sin_cos_formula() -> None:
    week = 13
    df = build_feature_df([1.5], "L2_TESCO", 0, week, {}, continuous_week=0)
...
def test_week_none_omits_sin_cos() -> None:
    df = build_feature_df([1.5], "L2_TESCO", 0, None, {}, continuous_week=0)
    assert "week_sin" not in df.columns
    assert "week_cos" not in df.columns


def test_attrs_are_passed_through() -> None:
    attrs = {"top_brand": "FANTA", "pack_size_internal": 330}
    df = build_feature_df([1.5], "L2_TESCO", 1, 1, attrs, continuous_week=50)
